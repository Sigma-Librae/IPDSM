function dA = odeMDomCW_EAS_NER(t,Init,p,S,modelMeta,startTime)
arguments
  t %#ok<INUSA> 
...
% special cases)
for i = 4:n-3
  dA(i) = (rate(i-2)*(Init(i-1)-Init(i)) + rate(i-2)*(Init(i+1)-Init(i))-p.k_ner*Init(i))*nS;
end
...
%NER
dA(end) = sum(Init(2:end-2))*p.k_ner;
